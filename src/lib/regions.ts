/**
 * 문서 F-02-01 — 활동 지역은 시/도 + 시/군/구 단위까지만 등록한다.
 * 표기 불일치로 매칭이 누락되지 않도록 자유 텍스트가 아닌 이 목록에서만 고르게 한다.
 *
 * 일반구를 둔 시(수원시 장안구 등)는 시 단위까지만 담았다.
 * 같은 시 안에서 구가 갈려 매칭이 나뉘는 편보다 시 단위로 묶이는 편이 매칭에 유리하다.
 */
...
/** 검색 입력과 대조할 "시/도 시/군/구" 평면 목록 */
export const REGION_OPTIONS: readonly Region[] = Object.entries(REGIONS).flatMap(
  ([sido, sigungus]) => sigungus.map((sigungu) => ({ sido, sigungu })),
);

export function isValidRegion(sido: string, sigungu: string): boolean {
  return REGIONS[sido]?.includes(sigungu) ?? false;
}

/** 세종처럼 시/도와 시/군/구가 같은 경우 중복 표기를 피한다. */
