...
리스트에 뜰 팀이 있어야 화면을 확인할 수 있다. 표본 팀·매칭 조건을 넣고 지우는 스크립트를 둔다.

```bash
npm run seed         # 표본 팀 18개 + 매칭 조건 34건 (다시 실행하면 지우고 새로 넣는다)
npm run seed:clean   # 표본 데이터만 삭제
```

지역·레벨·시간·비용이 필터마다 하나씩은 걸리도록 흩어 두었고, 경기 장소는 카카오 로컬 API 로 좌표를 받아
"내 주변" 반경도 확인할 수 있다. 팀 이름은 `테스트 ` 로 시작하고 `description` 이 표본 표식이라 실제 팀은 건드리지 않는다.

> GPS 를 끄면 문서 7.1 에 따라 **내 팀 활동 지역(시/군)** 과 같은 팀만 보인다.
> 표본은 수원시·성남시에 각각 넉넉히 넣어 두 계정 모두 기본 화면이 비지 않게 했다.
> 표본 팀에는 팀원을 넣지 않는다 — 팀원으로 넣으면 그 계정에게는 "내 팀" 이 되어 목록에서 빠지기 때문(F-05-05).
...
| `npm run seed` / `seed:clean` | 표본 팀·매칭 조건 넣기 / 지우기 |
